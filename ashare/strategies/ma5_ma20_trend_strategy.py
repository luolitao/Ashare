...
class MA5MA20StrategyRunner:
    ...
    def _load_daily_kline(self, codes: List[str], end_date: dt.date) -> pd.DataFrame:
        ...
        df = self.data_repo.load_daily_kline(
            tbl,
            codes,
            start_date_s,
            end_date_s,
    ...
    def _load_fundamentals_latest(self) -> pd.DataFrame:
        return self.data_repo.load_fundamentals_latest()

    def _load_stock_basic(self) -> pd.DataFrame:
        df = self.data_repo.load_stock_basic()
        if not df.empty and "ipoDate" in df.columns:
    ...
    def _attach_chip_factors(self, signals: pd.DataFrame) -> pd.DataFrame:
        ...
        chip_table_exists = self.store.table_exists(TABLE_STRATEGY_CHIP_FILTER)
    ...
    def _table_exists(self, table: str) -> bool:
        return self.store.table_exists(table)
    ...
    def _write_indicator_daily(
        ...
        *,
        ...
        self.store.write_indicator_daily(
            latest_date,
            signals,
            codes,
    ...
    def _write_signal_events(
        ...
        *,
        ...
        self.store.write_signal_events(
            latest_date,
            signals,
            codes,
            scope_override=scope_override,
    ...
    def _refresh_ready_signals_table(self, sig_dates: List[dt.date]) -> None:
        self.store.refresh_ready_signals_table(sig_dates)
